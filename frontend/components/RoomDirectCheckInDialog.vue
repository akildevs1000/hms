...
export default {
  ...
  async created() {
    ...
    this.temp = {
      ...
      room_type: this.reservation.room_type.name,
  ...
  methods: {
    ...
    add_room({
      ...
      if (!priceList) {
        this.$swal("Error!", "No Price found for the room", "error");
        return;
      }

      if (!this.room_type_object || !this.multipleRoomId) {
        this.$swal("Error!", "Room type or Room not selected", "error");
        return;
      }

      if (!no_of_adult) {
        this.$swal("Error!", "Adult must be selected", "error");
        return;
      }
      ...
      if (!selected_food_plan) {
        this.$swal("Error!", "Food plan not found!", "error");
        return;
      }
      ...
      this.$swal("Success!", "success selected room", "success");
    ...
    get_available_rooms(item) {
      ...
      this.$axios
        ...
        .then(({ data }) => {
          ...
          this.selectRoom({
            name: this.reservation.room_type.name,
            room_no: this.reservation.room_no,
          });
          this.getDays();
        });
