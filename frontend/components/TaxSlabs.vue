                    ...
                    <v-container>
                        <v-row>
                            ...
                            <v-col md="12" cols="12">
                                ...
                                <v-text-field :disabled="viewMode" v-model="editedItem.start_price" outlined dense small
                                    :hide-details="true" placeholder="Start Price" type="number"></v-text-field>
                                <span dense v-if="errors && errors.start_price" class="error--text">{{
                                    errors.start_price[0]
                                }}</span>
                            </v-col>
                            <v-col md="12" cols="12">
                                <label>End Price</label>
                                <v-text-field :disabled="viewMode" v-model="editedItem.end_price" outlined dense small
                                    :hide-details="true" placeholder="End Price" type="number"></v-text-field>
                                <span dense v-if="errors && errors.end_price" class="error--text">{{
                                    errors.end_price[0]
                                }}</span>
                            ...
                            <v-col md="12" cols="12">
                                <label> Tax %</label>
                                <v-text-field :disabled="viewMode" v-model="editedItem.tax" outlined dense small
                                    :hide-details="true" placeholder="%" type="number"></v-text-field>
                ...
                <v-col cols="12">
                    <v-data-table dense :headers="headers_table" :items="data" :loading="loading" :options.sync="options"
                        ...
                        <template v-slot:item.options="{ item }"
                            v-if="can('setting_tax_view') || can('setting_tax_edit') || can('setting_tax_delete')">
                            <v-menu bottom left>
                                <template v-slot:activator="{ on, attrs }">
                                ...
                                <v-list width="120" dense>
                                    <v-list-item v-if="can('setting_tax_view')" @click="editItem(item, true)">
                                        <v-list-item-title style="cursor: pointer">
                                            <v-icon color="primary" small> mdi-eye </v-icon>
                                            View
                                        </v-list-item-title>
                                    </v-list-item>
                                    <v-list-item v-if="can('setting_tax_edit')" @click="editItem(item, false)">
                                        <v-list-item-title style="cursor: pointer">
                                            <v-icon color="secondary" small> mdi-pencil </v-icon>
                                            Edit
                                        </v-list-item-title>
                                    </v-list-item>
                                    <v-list-item v-if="can('setting_tax_delete')" @click="deleteItem(item)">
                                        <v-list-item-title style="cursor: pointer">
...
export default
    {
        data: () => ({
            ...
            { text: "Start Price", value: "start_price", align: "left", sortable: true, filterable: true, },
            { text: "End Price", value: "end_price", align: "left", sortable: true, filterable: true, filterSpecial: true },
            { text: "Tax %", value: "tax", key: "tax", align: "left", sortable: true, filterable: true, filterSpecial: true },
        ...
        methods: {
            ...
            save() {
                ...
                if (this.editedItemIndex != -1) {
                    ...
                    this.$axios.post(`${this.endpoint}`, options.params).then(({ data }) => {
                        if (data.status) {
                            this.getDataFromApi();
                            this.errors = {};
                            this.editedItem = {};
