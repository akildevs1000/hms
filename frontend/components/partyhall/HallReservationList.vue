    ...
    <v-card class="mb-5 rounded-md mt-3" elevation="0">
      ...
      >
        ...
        <template v-slot:item.res_number="item">
          <span
            class="blue--text"
            @click="goToRevView(item.item.booking)"
        ...
        <template v-slot:item.view="item">
          <v-icon
            @click="viewCustomerBilling(item.item.booking)"
            x-small
            color="primary"
            class="mr-2"
          >
        ...
        <template v-slot:item.payment="item">
          <v-icon
            v-if="
              ...
              can('checkout_edit')
            "
            @click="get_payment(item.item.booking)"
            x-small
            color="primary"
            class="mr-2"
          >
        ...
        <template v-slot:item.invoice="item">
          <v-icon
            @click="redirect_to_invoice(item.item.booking.id)"
            x-small
            color="primary"
            class="mr-2"
          >
...
export default {
  ...
  methods: {
    ...
    process(type, model) {
      ...
      let url = process.env
        .BACKEND_URL`${type}?company_id=${comId}&from=${from}&to=${to}&search${search}&source${newSource}&r_type=${model}&guest_mode=${guest_mode}`;
      console.log(url);
      let element = document.createElement("a");
      element.setAttribute("target", "_blank");
