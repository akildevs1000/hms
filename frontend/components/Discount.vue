<template>
  <div class="text-center">
    <v-dialog v-model="discountPopUp" width="350">
      <v-card>
        <v-alert class="primary" dense dark>
          <v-row>
            <v-col> Set Discount </v-col>
            <v-col>
              <div class="text-right">
                <v-icon @click="discountPopUp = false">mdi-close</v-icon>
              </div>
            </v-col>
          </v-row>
        </v-alert>
...
export default {
  ...
  methods: {
    calc() {
      ...
      if (dt == "direct") {
        this.$emit("discountAbleAmount", Math.abs(dv));
