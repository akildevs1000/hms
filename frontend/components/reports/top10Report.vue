<template>
  <div
    v-if="
      can('management_top_10_customers_access') &&
      can('management_top_10_customers_view')
    "
  >
        ...
        <v-col cols="8">
            ...
            <v-tooltip top color="primary">
              <template v-slot:activator="{ on, attrs }">
                <v-btn
                  ...
                  v-on="on"
                  @click="
                    process('revenue_customer_wise_report_print', endpoint)
                  "
                >
            ...
            <v-tooltip top color="primary">
              <template v-slot:activator="{ on, attrs }">
                <v-btn
                  ...
                  v-on="on"
                  @click="
                    process('revenue_customer_wise_report_download', endpoint)
                  "
                >
          ...
          >
            ...
            <template v-slot:item.name="{ item }">
              <a @click="getToCheckoutPage(item)"> {{ item.title }}</a>
            </template>
  ...
  methods: {
    ...
    getPercentage(customer_total_price) {
      if (this.total_price > 0)
        return Math.round((customer_total_price / this.total_price) * 100);
      else return 0;
    },
    getRoomsCount(rooms) {
    ...
    getDataFromApi(url = this.endpoint) {
      ...
      this.$axios
        .get("get_report_top_ten_customers", options)
        .then(({ data }) => {
          ...
          this.data_table.forEach((item) => {
            ...
            if (counter <= 9) {
              ...
              this.chartOptions.customLabel[counter] =
                item.title +
                "<br/>Total Amount: " +
                this.getPriceFormat(item.customer_total_price) +
                "<br/>No.of Visits: " +
                item.number_of_visits +
                "<br/>No.of Rooms: " +
                rooms;
