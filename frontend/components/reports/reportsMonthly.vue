<template>
  <div
    v-if="
      can('management_revenue_report_access') &&
      can('management_revenue_report_view')
    "
  >
    <v-row>
      <v-col md="12" lg="6" cols="12">
        ...
        >
          <template v-slot:item.month_name="{ item }">
            <a @click="goToDailyReport(item.year_number, item.month_number)">{{
              item.month
            }}</a>
          </template>
          ...
          <template slot="body.append">
            <tr>
              ...
              <td class="text-right font-weight-bold">
                {{ grandTotal.totalManagementExpenses }}
              </td>
              <td class="text-right font-weight-bold">
  ...
  methods: {
    ...
    printTable() {
      let htmlHeaderContent =
        "<center><h4>Revenue Report - Month wise -  " +
        this.year +
        "</h4></center>";
      ...
      printWindow.document.write(
        document.querySelector(".v-data-table").outerHTML
      );
      printWindow.document.write(
        "<style>.text-right{text-align:right;} td,th {border-top:1px solid #DDD;border-left:1px solid #DDD} table{border-right:1px solid #DDD;border-bottom:1px solid #DDD;width:100%} body{width:95%}</style></body></html>"
      );
    ...
    goToDailyReport(year, month) {
      //this.getDataFromApi();
      this.$emit("goToDailyReportTab", {
        tab: 1,
        filter_from_date: year + "-" + month + "-01",
        filter_to_date:
          year + "-" + month + "-" + this.getTotalDays(year, month),
      });
    ...
    getDataFromApi(url = this.endpoint) {
      ...
      this.$axios
        .get("get_report_monthly_wise_group", options)
        .then(({ data }) => {
          ...
          this.data_table.forEach((item) => {
            this.barSeriesNew[0]["data"][counter] = parseInt(
              item.income.replaceAll(",", "")
            );
            this.barSeriesNew[1]["data"][counter] = parseInt(
              item.total_expenses.replaceAll(",", "")
            );
            ...
            this.barChartOptionsNew.customLabel[counter] =
              "<table>" +
              "<tr><td>Income</td><td style='text-align:right;color:green'> :  " +
              item.income +
              "</td></tr> " +
              "<tr><td>Non-Mng Expenses</td><td style='text-align:right;color:red'>   - " +
              item.expenses +
              "</td></tr> " +
              "<tr><td>Management Expenses</td><td style='text-align:right;color:red'>  - " +
              item.management_expenses +
              "</td></tr> " +
              "<tr><td>Proffit</td><td style='text-align:right; '>   = " +
              item.profit +
              "</td></tr>" +
              "<tr><td>Rooms Sold</td><td> :  " +
              item.sold +
              "</td></tr> " +
              "</table > ";
    ...
    process(type, model) {
      let url = `${process.env.BACKEND_URL}${type}?company_id=${this.$auth.user.company.id}&filter_from_date=${this.filter_from_date}&filter_to_date=${this.filter_to_date}`;
      console.log(url);
