<template>
  <v-card>
    <v-toolbar class="rounded-md" color="background" dense flat dark>
      <span>Direct Check In Information</span>
      <v-spacer></v-spacer>
      <v-icon dark @click="$emit(`close-dialog`)"> mdi-close </v-icon>
    </v-toolbar>
    <v-card-text>
      <v-tabs v-model="activeTab">
        ...
        <v-tab-item class="mt-5">
          ...
          <v-row class="text-right mb-3">
            <v-col>
              <RoomDirectCheckInDialog
                :key="activeTab"
                v-if="!priceListTableView.length && !activeTab == 0"
                :reservation="{ ...reservation, room_id: reservation.id }"
                label="Room"
                @tableData="handleTableData"
              />
              <v-btn small class="blue" @click="store" :loading="subLoad" dark
...
export default {
  ...
  methods: {
    ...
    nextTab() {
      ...
      this.RoomDirectCheckInCheck = true;
    ...
    async storeCheckIn(data) {
      await this.$axios.post(`direct_check_in_room`, {
        room_id: this.reservation.id,
        booking_id: data.data,
      });
