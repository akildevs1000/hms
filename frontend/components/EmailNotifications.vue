        ...
        <v-col cols="12">
          ...
          >
              ...
              <tr v-if="isFilter">
                <td v-for="header in headers" :key="header.text">
                  ...
                  <v-autocomplete
                    ...
                    dense
                    :hide-details="true"
                    :items="EmailTypesForSelectOptions"
                    item-text="name"
                    item-value="id"
                  >
                  </v-autocomplete>
            ...
            <template v-slot:item.report_name="{ item }">
              <span v-for="access in item.report_type_access"
                >{{ access.report_type.name }}<br />
              </span>
            </template>
            <template
              v-slot:item.options="{ item }"
              v-if="
                can('settings_Emails_view') ||
                can('settings_Emails_edit') ||
                can('settings_Emails_delete')
              "
            >
              <v-menu bottom left>
                <template v-slot:activator="{ on, attrs }">
                ...
                <v-list width="120" dense>
                  ...
                  <v-list-item
                    v-if="can('settings_Emails_delete')"
...
export default {
  ...
  methods: {
    ...
    editItem(item, viewMode = false) {
      ...
      this.selectedReportTypes = item.report_type_access.map(
        (item) => item.notification_report_types_id
      );
    ...
    save() {
      if (this.editedItemIndex != -1) {
        ...
        this.$axios
          .post(`${this.endpoint}/storeSingle`, options.params)
          .then(({ data }) => {
            if (data.status) {
              this.getDataFromApi();
              this.errors = {};
