<template>
  <v-card v-if="isPageLoad">
    <v-container fluid>
              ...
              <v-col md="6" class="p-0 m-0">
                <h1 class="big-screen laptop-font-grid ipad-font-qty-grid">
                  {{
                    ...
                    (reservedWithoutAdvance && reservedWithoutAdvance.length) ||
                    0
                  }}
                </h1>
              </v-col>
      ...
      <v-row v-if="availableRooms.length">
        <v-col>
          ...
          <v-btn
            ...
            :elevation="0"
          >
            <v-icon left>mdi-home</v-icon> <span>{{ room.room_no }}</span>
          </v-btn>
        </v-col>
      </v-row>
      <v-row v-if="blockedRooms.length">
        <v-col>
          <div class="ml-2">Blocked</div>
          <v-btn
            v-for="(room, index) in blockedRooms"
            :key="index"
      ...
      <v-row v-if="notAvailableRooms.length">
        <v-col>
          <div class="ml-2">Occupied</div>
          <v-btn
            color="red darken-2"
            class="ma-2 white--text"
          ...
          >
            ...
            <!-- <div>{{noAvailableRoom}}</div> -->
          </v-btn>
        </v-col>
      </v-row>
      <v-row v-if="expectCheckOut.length">
        <v-col>
          <div class="ml-2">Expect Checkout</div>
          <v-btn
            v-for="(room, index) in expectCheckOut"
            :key="index"
            ...
            :elevation="0"
          >
            <v-icon left>mdi-bed</v-icon> <span>{{ room.room_no }}</span>
          </v-btn>
        </v-col>
      </v-row>
      <v-row v-if="expectCheckIn.length">
        <v-col>
          <div class="ml-2">Expect Arrival</div>
          <v-btn
            v-for="(room, index) in expectCheckIn"
            :key="index"
            color="success"
            class="ma-2 white--text"
            @contextmenu="makeNewBooking($event, room)"
      ...
      <v-row v-if="reservedWithoutAdvance.length">
        <v-col>
          <div class="ml-2">Reserved</div>
          <v-btn
            v-for="(room, index) in reservedWithoutAdvance"
            :key="index"
            color="blue darken-2"
            class="ma-2 white--text"
            @contextmenu="makeNewBooking($event, room)"
...
export default {
  ...
  data() {
    return {
      chartItems: [
        {
          labels: [`Arrived (14)`, `Pending (14)`],
          series: [14, 14],
          size: "100%",
          heading: "Arrival",
        },
        {
          labels: [`Checkout (14)`, `Pending (04)`],
          series: [14, 4],
          size: "100%",
          heading: "Checkout",
        },
        {
          labels: [`Adult (14)`, `Children (04)`],
          series: [14, 4],
          size: "100%",
          heading: "In House",
        },
        ...
        // },
        {
          labels: [`Breakfast( 04)`, `Lunch (24)`, `Dinner (01)`],
          series: [4, 24, 1],
          size: "260px",
          heading: "Food Order",
        },
